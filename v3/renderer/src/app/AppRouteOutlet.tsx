...
export function AppRouteOutlet(props: AppRouteOutletProps) {
  ...
  if (navigation.view === "home") {
    ...
    content = <SettingsView section={navigation.settingsSection} advancedOpen={onboarding.advancedSettingsOpen} busy={operations.busy} activeCapture={activeCapture} combinedCaptureAvailable={combinedCaptureAvailable} statuses={{ core: runtime.coreStatus, consent: runtime.consentStatus, capture: runtime.captureStatus, vault: runtime.vaultStatus, updates: runtime.updateStatus, retention: runtime.retentionStatus, transcription: runtime.transcriptionStatus }} models={localAi.models} selectedModel={localAi.selectedModel} defaultModel={DEFAULT_MODEL} aiMode={localAi.aiMode} aiModeStatus={localAi.aiModeStatus} instructSetupOpen={localAi.instructSetupOpen} instructAssetsReady={localAi.instructAssetsReady} instructRunnerAsset={localAi.instructRunnerAsset} instructModelAsset={localAi.instructModelAsset} instructAssetKind={localAi.instructAssetKind} instructExpectedSha256={localAi.instructExpectedSha256} instructAssetError={localAi.instructAssetError} licenseStatus={license.status} licensePortalInfo={license.portalInfo} licenseActive={license.active} privacyReceipt={meeting.privacyReceipt} networkCapabilities={runtime.networkCapabilities} custodyItems={custodyItems} onSectionChange={navigation.setSettingsSection} onToggleAdvanced={onboarding.toggleAdvancedSettings} onVerifyModel={() => void localAi.verifyModel()} onImportModel={() => void localAi.importModel()} onSelectedModelChange={localAi.setSelectedModel} onAiModeChange={localAi.setAiMode} onInstructSetupOpenChange={localAi.setInstructSetupOpen} onInstructAssetKindChange={(kind) => { localAi.setInstructAssetKind(kind); localAi.setInstructAssetError(""); }} onInstructExpectedShaChange={(value) => { localAi.setInstructExpectedSha256(value); localAi.setInstructAssetError(""); }} onImportInstructAsset={() => void localAi.importInstructAsset()} onRefreshLicense={() => void license.reload()} onDeactivateLicense={() => void onboarding.deactivate()} onAcknowledgeMic={() => void onboarding.acknowledgeMic()} onAcknowledgeSystem={() => void onboarding.acknowledgeSystem()} onRecordSystem={() => void captureActions.startSystem()} onRecordBoth={() => void captureActions.startCombined()} onOpenExport={() => navigation.setView("export")} onRefreshLocalSettings={() => void onboarding.refreshLocalSettings()} />;
  } else {
