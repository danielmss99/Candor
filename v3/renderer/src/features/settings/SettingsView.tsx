...
export function SettingsView(props: SettingsViewProps) {
  ...
  const renderPanel = () => {
    ...
    if (props.section === "privacy") return <div className="settings-panel-content" aria-label="Local custody"><header><h2>Privacy and diagnostics</h2><p>Facts reported by the local core</p></header><PrivacyReceipt receipt={props.privacyReceipt} network={props.networkCapabilities} /><dl className="settings-facts privacy-facts">{props.custodyItems.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><button type="button" className="secondary-button" onClick={props.onOpenDiagnostics}>Open full diagnostics</button></div>;
