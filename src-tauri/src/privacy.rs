...
pub struct PrivacySettings {
  ...
  /// Optional webhook URL called when a meeting is saved.
  pub webhook_url: Option<String>,
  /// Scaffold: enable local MCP server (not implemented — see docs/mcp-server.md).
...
pub fn load_privacy(app: &AppHandle) -> PrivacySettings {
  settings_path(app)
    ...
    .and_then(|v| {
      serde_json::from_value::<PrivacySettings>(v.get("privacy")?.clone()).ok()
    })
    .unwrap_or_default()
}
